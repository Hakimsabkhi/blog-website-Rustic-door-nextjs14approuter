...
const NosBlog: React.FC = () => {
  ...
  {blogs.map((blog, index) => (
    <div 
    key={index} 
    className={`w-full mb-8 overflow-hidden block rounded-xl border shadow-xl transition hover:border-orange-400/10 hover:shadow-orange-400/10 flex-1 ${index === 2 ? 'hidden sm:block  md:block' : ''}`}
  >
